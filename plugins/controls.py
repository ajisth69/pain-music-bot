...
async def _play_next_song(client, chat_id, next_song, requester_mention=None):
    """Resolve file, play it, send the player card, update state. Returns player_msg."""
...
async def resume_cmd(client, message: Message):
    ...
    data = playing_chats[chat_id]
    if not data["paused"]:
        return await message.reply("▶️  Already playing.")
...
async def stop_cmd(client, message: Message):
    ...
    await message.reply(f"⏹  **Stopped** by {message.from_user.mention}\n_Voice chat left._")
...
async def skip_cmd(client, message: Message):
    ...
    if not next_song:
        ...
        return await message.reply(
            ...
            "📋  _Queue is empty — leaving voice chat._"
        )
    ...
    try:
        ...
        await message.reply(f"❌  **Skip failed:** `{e}`")
...
async def queue_cmd(client, message: Message):
    ...
    if now:
        lines.append(
            f"▶️  **Now Playing**\n"
            f"    🎼  {now['title']}\n"
            f"    🎤  {now.get('artist','Unknown')}  ·  "
            f"`{format_time(now['duration'])}`"
        )
        lines.append("━━━━━━━━━━━━━━━━━━━━━━")
    else:
        lines.append("_Nothing is playing right now._")

    if q:
        for i, s in enumerate(q, 1):
            status = "⬇️" if s.get("downloading") else ("✅" if s.get("file_path") else "🕐")
            lines.append(
                f"`{i}.` {status}  **{s['title']}**\n"
    ...
    lines.append("━━━━━━━━━━━━━━━━━━━━━━")
    lines.append("✦  **PAIN !!**  ·  _LᴇᴛMᴇ Sᴏʟᴏ Hᴇʀ_ 🥀")

    await message.reply("\n".join(lines), reply_markup=get_player_markup(chat_id) if now else None)
